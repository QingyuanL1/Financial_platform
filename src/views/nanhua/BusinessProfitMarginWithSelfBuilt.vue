                ...
                <tbody>
                    ...
                    <template v-for="(item, index) in profitMarginData.customers" :key="`customer-${index}`">
                        <tr>
                            ...
                            <td class="border border-gray-300 px-4 py-2 text-right">
                                {{ formatPercentage(item.accumulated) }}
                            </td>
                            <td class="border border-gray-300 px-4 py-2 text-right">
                                {{ formatPercentage(calculateDeviation(item.yearlyPlan, item.accumulated)) }}
                            </td>
                        </tr>
                    </template>
                    ...
                    <tr class="bg-gray-50 font-bold">
                        ...
                        <td class="border border-gray-300 px-4 py-2 text-right">
                            {{ formatPercentage(totalData.accumulated) }}
                        </td>
                        <td class="border border-gray-300 px-4 py-2 text-right">
                            {{ formatPercentage(calculateDeviation(totalData.yearlyPlan, totalData.accumulated)) }}
                        </td>
                    </tr>
                </tbody>
            </table>
...
const fixedPlanData: ProfitMarginData = {
    customers: [
        { customerName: '一包项目', yearlyPlan: 14.54, current: 0, accumulated: 0 },
        { customerName: '二包项目', yearlyPlan: 15.50, current: 0, accumulated: 0 },
        { customerName: '域内合作项目', yearlyPlan: 8.00, current: 0, accumulated: 0 },
        { customerName: '域外合作项目', yearlyPlan: 5.48, current: 0, accumulated: 0 },
        { customerName: '新能源项目', yearlyPlan: 17.25, current: 0, accumulated: 0 },
        { customerName: '苏州项目', yearlyPlan: 6.00, current: 0, accumulated: 0 },
        { customerName: '抢修', yearlyPlan: 33.52, current: 0, accumulated: 0 },
        { customerName: '运检项目', yearlyPlan: 13.60, current: 0, accumulated: 0 },
        { customerName: '自建项目', yearlyPlan: 0, current: 0, accumulated: 0 }
    ]
}
...
// 计算偏差（累计 - 年度计划）
const calculateDeviation = (yearlyPlan: number, accumulated: number): number => {
    return (accumulated || 0) - (yearlyPlan || 0)
}
...
const totalData = computed(() => {
    ...
    total.accumulated = total.accumulated / profitMarginData.value.customers.length
...
const loadData = async (targetPeriod: string) => {
    try {
        ...
        if (result.data && result.data.customers) {
            // 直接使用后端返回的数据
            profitMarginData.value.customers = result.data.customers.map((item: any) => ({
