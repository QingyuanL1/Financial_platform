    ...
    try {
        ...
        for (let item of investmentData.value.items) {
            ...
            for (let m = 1; m < currentMonth; m++) {
                ...
                try {
                    const response = await fetch(`http://127.0.0.1:3000/nanhua-major-investment/${monthPeriod}`)
                    if (response.ok) {
...
const loadData = async (targetPeriod: string) => {
    try {
        const response = await fetch(`http://127.0.0.1:3000/nanhua-major-investment/${targetPeriod}`)
        if (!response.ok) {
...
const loadRemarksAndSuggestions = async (targetPeriod: string) => {
    try {
        const response = await fetch(`http://127.0.0.1:3000/forms/submission/${MODULE_IDS.NANHUA_MAJOR_INVESTMENT}/${targetPeriod}`)
        if (response.ok) {
            const result = await response.json()
...
const handleSave = async () => {
    try {
        const response = await fetch('http://127.0.0.1:3000/nanhua-major-investment', {
            method: 'POST',
            headers: {
