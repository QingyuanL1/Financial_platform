<template>
    <div class="max-w-[1500px] mx-auto bg-white rounded-lg shadow-lg p-6">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold">拓源存量结构与质量</h1>
    ...
    try {
        ...
        for (let item of inventoryData.value.items) {
            ...
            for (let m = 1; m < currentMonth; m++) {
                ...
                try {
                    const response = await fetch(`http://47.111.95.19:3000/tuoyuan-inventory-structure/${monthPeriod}`)
                    if (response.ok) {
...
const loadData = async (targetPeriod: string) => {
    try {
        const response = await fetch(`http://47.111.95.19:3000/tuoyuan-inventory-structure/${targetPeriod}`)
        if (!response.ok) {
...
const handleSave = async () => {
    try {
        const response = await fetch('http://47.111.95.19:3000/tuoyuan-inventory-structure', {
            method: 'POST',
            headers: {
