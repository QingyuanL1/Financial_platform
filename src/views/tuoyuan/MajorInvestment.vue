<template>
    <div class="max-w-[1500px] mx-auto bg-white rounded-lg shadow-lg p-6">
        ...
        <FormAttachmentAndRemarks 
            :module-id="MODULE_IDS.TUOYUAN_MAJOR_INVESTMENT"
            :period="period"
    ...
    try {
        ...
        for (let item of investmentData.value.items) {
            ...
            for (let m = 1; m < currentMonth; m++) {
                ...
                try {
                    const response = await fetch(`http://47.111.95.19:3000/tuoyuan-major-investment/${monthPeriod}`)
                    if (response.ok) {
...
const loadData = async (targetPeriod: string) => {
    try {
        const response = await fetch(`http://47.111.95.19:3000/tuoyuan-major-investment/${targetPeriod}`)
        if (!response.ok) {
...
const loadRemarksAndSuggestions = async (targetPeriod: string) => {
    try {
        const response = await fetch(`http://47.111.95.19:3000/forms/submission/${MODULE_IDS.TUOYUAN_MAJOR_INVESTMENT}/${targetPeriod}`)
        if (response.ok) {
            const result = await response.json()
...
const handleSave = async () => {
    try {
        const response = await fetch('http://47.111.95.19:3000/tuoyuan-major-investment', {
            method: 'POST',
            headers: {
        ...
        await recordFormSubmission(MODULE_IDS.TUOYUAN_MAJOR_INVESTMENT, period.value, { items: investmentData.value.items }, remarks.value, suggestions.value)
        alert('保存成功')
    } catch (error) {
